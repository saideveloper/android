...
public class MenuExpandableCourseAdapter extends BaseExpandableListAdapter {
	...
	public interface SelectedCourse {
		void selectedCourse(String selectedCourse);
	}

	SelectedCourse selectedCourseListener;

	public interface CourseAnimateOrderIcon {
		void animate();
	}

	CourseAnimateOrderIcon courseAnimateOrderIconListener;

	public MenuExpandableCourseAdapter(Activity activity,
		...
		this.courseAnimateOrderIconListener = courseAnimateOrderIconListener;
		this.selectedCourseListener = selectedCourseListener;

		font = Typeface.createFromAsset(activity.getAssets(),
				"Roboto-Regular.ttf");

		// listViewCache = new DebugListView[ parentItems.size() ];

		List<String> courseList = new ArrayList<String>();
		courseList.add("All Courses");

		try {
			List<MenuDetails> courses = DatabaseManager
					.getInstance().getMenuDetailsDao().queryBuilder()
					.orderBy("course", true).groupBy("course").query();

			for (MenuDetails course : courses) {
				courseList.add("" + course.getCourse());
			}

			dataAdapter = new ArrayAdapter<String>(activity,
					R.layout.spinner_list, courseList);
		...
		childViewHolder.layoutAdd.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (menuDetail.getModifierStatus().equalsIgnoreCase("n")) {
					...
					courseAnimateOrderIconListener.animate();
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		if (groupPosition == 0) {
			...
			courseSpinner = (Spinner) convertView.findViewById(R.id.spinner1);
			courseSpinner
					.setOnItemSelectedListener(new OnItemSelectedListener() {
						...
						public void onItemSelected(AdapterView<?> parent,
							...
							if (ExpandableMenuActivity.firsttime) {
								...
								ExpandableMenuActivity.selectedCoursePosition = pos;
								ExpandableMenuActivity.selectedCourse = parent
										.getItemAtPosition(pos).toString();
								ExpandableMenuActivity.firsttime = true;
								selectedCourseListener
										.selectedCourse(ExpandableMenuActivity.selectedCourse);
							}
						}
			...
			courseSpinner.setAdapter(dataAdapter);
			courseSpinner
					.setSelection(ExpandableMenuActivity.selectedCoursePosition);
			// }

			// List<String> Courses = new ArrayList<String>();
			// Courses.add("All Categories");
			//
			// try {
			// ArrayList<MenuDetails> Course = (ArrayList<MenuDetails>)
			// DatabaseManager.getInstance()
			// .getMenuDetailsDao().queryBuilder().distinct()
			// .selectColumns("Course").query();
			// for (MenuDetails menuDetail : Course) {
			// Courses.add("" + menuDetail.getCourse());
			// }
			// } catch (SQLException e) {
			// e.printStackTrace();
			// }
			...
			// activity, R.layout.spinner_list, Courses);
	...
	public void addToOrder(MenuDetails menuDetail, ChildViewHolder viewHolder) {
		try {
			...
			if (orderMenuDetailsList.size() > 0) {
				System.out.println("item incremented!");
				orderMenuDetail = orderMenuDetailsList.get(0);
